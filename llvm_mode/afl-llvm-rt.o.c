  ...
  while (1) {
    ...
    if(mode[0] == '0') {
        ...
        for(int i = 0; i < pipe_argc -1; i++) {
            ...
            /*dprintf(fd, "%d: pipe_argc: %d, ", i, pipe_argc-1);*/
            /*dprintf(fd, "argv_tmp %s\n", argv_tmp);*/
            sprintf(argv[i], "%s", argv_tmp);
        }
        /*dprintf(fd, "=========\n");*/
        null_content = argv[pipe_argc - 1];
        null_pos = pipe_argc - 1;
        argv[pipe_argc - 1] = NULL;
        pipe_argc --; 
    }
    /*close(fd);*/
...
__attribute__((constructor(CONST_PRIO))) void __afl_auto_init(int argc, char** argv) {
